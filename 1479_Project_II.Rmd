...
for(i in c(1:10)){
  ...
  star_ratings <- page %>%
    html_elements(xpath = "//*[@data-hook='review-star-rating']") %>%
    html_text()
  #review_stars_all=append(review_stars_all, star_ratings)
  review_dates <- page %>%
    html_elements(xpath = "//*[@data-hook='review-date']") %>%
    html_text()
  # Use str_match on the vecto r review_dates
  dates <- str_match(review_dates, "on (\\d+\\s[A-Za-z]+\\s\\d{4})")[, 2]
  # Convert the extracted dates to a standard date format
  formatted_dates <- as.Date(dates, format = "%d %B %Y", locale = "de")
  #review_dates_all=append(review_dates_all, formatted_dates)
...
#df<-data.frame(review_dates_all, review_text_all, review_stars_all)
```
...
scrape_reviews_multiple_pages <- function(base_url, num_pages) {
  all_reviews <- character(0)
  
  for (i in 1:num_pages) {
    current_url <- paste0(base_url, "&pageNumber=", i)
    reviews_on_page <- scrape_reviews(current_url)
    
    all_reviews <- c(all_reviews, reviews_on_page)
...
# Example usage
base_url <- "https://www.amazon.com/iPhone-13-Pro-256GB-Graphite/product-reviews/B0BGYBX3LK/ref=cm_cr_arp_d_paging_btm_next_2?ie=UTF8&reviewerType=all_reviews&pageNumber="
num_pages <- 5  # Set the number of pages to scrape

all_reviews <- scrape_reviews_multiple_pages(base_url, num_pages)
